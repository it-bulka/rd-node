...
export class TeaService {
  ...
  async getAll({ minRating, page, pageSize }: QueryGetAllDto = {}): Promise<PaginatedTea> {
    ...
    if(minRating){  list = this._filterByRating(list, minRating) }
  ...
  _paginate({ list, pageSize = 5, page = 1}: PaginateArg): PaginatedTea {
    ...
    if(totalPages < page){ throw new BadRequestException(`Page ${page} is out of range. Only ${totalPages} pages are available`) }
